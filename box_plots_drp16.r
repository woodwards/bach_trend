...
p1 <- bplot1(sampledata, 'chem1fast0', expression('Fast DRP Conc'~(mg~L^{-1})*'\n'), seq(0,0.8,0.2), median_colour=NA, median_fill=fcol[3], median_alpha=0.2)
p2 <- bplot1(sampledata, 'chem1med0', expression('Medium DRP Conc'~(mg~L^{-1})*'\n'), seq(0,0.8,0.2), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
p3 <- bplot1(sampledata, 'chem1slow0', expression('Slow DRP Conc'~(mg~L^{-1})*'\n'), seq(0,0.8,0.2), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
p4 <- bplot1(sampledata, 'chem2fast0', expression('Fast NO3-N Conc'~(mg~L^{-1})*'\n'), seq(0,6,1), median_colour=NA, median_fill=fcol[3], median_alpha=0.2)
p5 <- bplot1(sampledata, 'chem2med0', expression('Medium NO3-N Conc'~(mg~L^{-1})*'\n'), seq(0,6,1), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
p6 <- bplot1(sampledata, 'chem2slow0', expression('Slow NO3-N Conc'~(mg~L^{-1})*'\n'), seq(0,6,1), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
...
p1 <- bplot1(sampledata, 'chem1fast1', expression('Fast DRP Conc'~(mg~L^{-1})*'\n'), seq(0,0.8,0.2), median_colour=NA, median_fill=fcol[3], median_alpha=0.2)
p2 <- bplot1(sampledata, 'chem1med1', expression('Medium DRP Conc'~(mg~L^{-1})*'\n'), seq(0,0.8,0.2), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
p3 <- bplot1(sampledata, 'chem1slow1', expression('Slow DRP Conc'~(mg~L^{-1})*'\n'), seq(0,0.8,0.2), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
p4 <- bplot1(sampledata, 'chem2fast1', expression('Fast NO3-N Conc'~(mg~L^{-1})*'\n'), seq(0,6,1), median_colour=NA, median_fill=fcol[3], median_alpha=0.2)
p5 <- bplot1(sampledata, 'chem2med1', expression('Medium NO3-N Conc'~(mg~L^{-1})*'\n'), seq(0,6,1), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
p6 <- bplot1(sampledata, 'chem2slow1', expression('Slow NO3-N Conc'~(mg~L^{-1})*'\n'), seq(0,6,1), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
plotbox <- plot_grid(p1, p4, p2, p5, p3, p6, nrow=3, align="hv")
# print(plotbox)
...
p1 <- bplot1(sampledata, 'chem1fast0', expression('Fast DRP Conc0'~(mg~L^{-1})*'\n'), seq(0,0.8,0.2), median_colour=NA, median_fill=fcol[3], median_alpha=0.2)
p2 <- bplot1(sampledata, 'chem1med0', expression('Medium DRP Conc0'~(mg~L^{-1})*'\n'), seq(0,0.8,0.2), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
p3 <- bplot1(sampledata, 'chem1slow0', expression('Slow DRP Conc0'~(mg~L^{-1})*'\n'), seq(0,0.8,0.2), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
p4 <- bplot1(sampledata, 'chem1fast1', expression('Fast DRP Conc1'~(mg~L^{-1})*'\n'), seq(0,0.8,0.2), median_colour=NA, median_fill=fcol[3], median_alpha=0.2)
p5 <- bplot1(sampledata, 'chem1med1', expression('Medium DRP Conc1'~(mg~L^{-1})*'\n'), seq(0,0.8,0.2), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
p6 <- bplot1(sampledata, 'chem1slow1', expression('Slow DRP Conc1'~(mg~L^{-1})*'\n'), seq(0,0.8,0.2), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
plotbox <- plot_grid(p1, p4, p2, p5, p3, p6, nrow=3, align="hv")
# print(plotbox)
...
p1 <- bplot1(sampledata, 'chem2fast0', expression('Fast NO3-N Conc0'~(mg~L^{-1})*'\n'), seq(0,6,1), median_colour=NA, median_fill=fcol[3], median_alpha=0.2)
p2 <- bplot1(sampledata, 'chem2med0', expression('Medium NO3-N Conc0'~(mg~L^{-1})*'\n'), seq(0,6,1), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
p3 <- bplot1(sampledata, 'chem2slow0', expression('Slow NO3-N Conc0'~(mg~L^{-1})*'\n'), seq(0,6,1), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
p4 <- bplot1(sampledata, 'chem2fast1', expression('Fast NO3-N Conc1'~(mg~L^{-1})*'\n'), seq(0,6,1), median_colour=NA, median_fill=fcol[3], median_alpha=0.2)
p5 <- bplot1(sampledata, 'chem2med1', expression('Medium NO3-N Conc1'~(mg~L^{-1})*'\n'), seq(0,6,1), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
p6 <- bplot1(sampledata, 'chem2slow1', expression('Slow NO3-N Conc1'~(mg~L^{-1})*'\n'), seq(0,6,1), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
plotbox <- plot_grid(p1, p4, p2, p5, p3, p6, nrow=3, align="hv")
# print(plotbox)
...
bplot2 <- function(sampledata, varname0, varname1, ylabel, ybreaks, yref=0, box_fill="white",
  ...
  if (is.na(median_fill)){
    sampledata2 <- sampledata %>%
      ...
      select(all_of(c("varname0", "varname1")), catchnamef, setseqf, setseq) %>%
...
print("making dconc box") # dconc box ####
p1 <- bplot2(sampledata, 'chem1fast0', 'chem1fast1', expression('Fast DRP Conc'~(mg~L^{-1})*'\n'), seq(0,0.8,0.2), median_colour=NA, median_fill=fcol[3], median_alpha=0.2)
p2 <- bplot2(sampledata, 'chem1med0', 'chem1med1', expression('Medium DRP Conc'~(mg~L^{-1})*'\n'), seq(0,0.8,0.2), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
p3 <- bplot2(sampledata, 'chem1slow0', 'chem1slow1', expression('Slow DRP Conc'~(mg~L^{-1})*'\n'), seq(0,0.8,0.2), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
p4 <- bplot2(sampledata, 'chem2fast0', 'chem2fast1', expression('Fast NO3-N Conc'~(mg~L^{-1})*'\n'), seq(0,6,1), median_colour=NA, median_fill=fcol[3], median_alpha=0.2)
p5 <- bplot2(sampledata, 'chem2med0', 'chem2med1', expression('Medium NO3-N Conc'~(mg~L^{-1})*'\n'), seq(0,6,1), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
p6 <- bplot2(sampledata, 'chem2slow0', 'chem2slow1', expression('Slow NO3-N Conc'~(mg~L^{-1})*'\n'), seq(0,6,1), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
plotbox <- plot_grid(p1, p4, p2, p5, p3, p6, nrow=3, align="hv")
# print(plotbox)
...
pt1 <- bplot1(sampledata, 'totalTPloadkg', expression('Total DRP Yield '~(kg~ha^{-1}~y^{-1})*'\n'), seq(0,1.6,0.2), median_colour=NA, median_fill=tcol, median_alpha=0.2)
varname <- 'totalTNloadkg'
pt2 <- bplot1(sampledata, 'totalTNloadkg', expression('Total NO3-N Yield '~(kg~ha^{-1}~y^{-1})*'\n'), seq(0,20,5), median_colour=NA, median_fill=tcol, median_alpha=0.2)
varname <- 'totalflowmm'
...
p1 <- bplot1(sampledata, 'fastTPloadkg', expression('Fast DRP Yield '~(kg~ha^{-1}~y^{-1})*'\n'), seq(0,0.5,0.1), median_colour=NA, median_fill=fcol[3], median_alpha=0.2)
p2 <- bplot1(sampledata, 'medTPloadkg', expression('Medium DRP Yield '~(kg~ha^{-1}~y^{-1})*'\n'), seq(0,0.5,0.1), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
p3 <- bplot1(sampledata, 'slowTPloadkg', expression('Slow DRP Yield '~(kg~ha^{-1}~y^{-1})*'\n'), seq(0,0.5,0.1), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
p4 <- bplot1(sampledata, 'fastTPloadpc', 'Fast DRP Yield Fraction\n', seq(0,1,0.2), median_colour=NA, median_fill=fcol[3], median_alpha=0.2)
p5 <- bplot1(sampledata, 'medTPloadpc', 'Medium DRP Yield Fraction\n', seq(0,1,0.2), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
varname <- 'slowTPloadpc'
p6 <- bplot1(sampledata, 'slowTPloadpc', 'Slow DRP Yield Fraction\n', seq(0,1,0.2), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
plotbox <- plot_grid(p1, p4, p2, p5, p3, p6, nrow=3, align="hv")
...
p1 <- bplot1(sampledata, 'fastTNloadkg', expression('Fast NO3-N Yield '~(kg~ha^{-1}~y^{-1})*'\n'), seq(0,10,2), median_colour=NA, median_fill=fcol[3], median_alpha=0.2)
p2 <- bplot1(sampledata, 'medTNloadkg', expression('Medium NO3-N Yield '~(kg~ha^{-1}~y^{-1})*'\n'), seq(0,10,2), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
p3 <- bplot1(sampledata, 'slowTNloadkg', expression('Slow NO3-N Yield '~(kg~ha^{-1}~y^{-1})*'\n'), seq(0,10,2), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
p4 <- bplot1(sampledata, 'fastTNloadpc', 'Fast NO3-N Yield Fraction\n', seq(0,1,0.2), median_colour=NA, median_fill=fcol[3], median_alpha=0.2)
p5 <- bplot1(sampledata, 'medTNloadpc', 'Medium NO3-N Yield Fraction\n', seq(0,1,0.2), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
p6 <- bplot1(sampledata, 'slowTNloadpc', 'Slow NO3-N Yield Fraction\n', seq(0,1,0.2), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
plotbox <- plot_grid(p1, p4, p2, p5, p3, p6, nrow=3, align="hv")
...
p2 <- bplot1(sampledata, 'medTPloadkg', expression('Medium DRP Yield '~(kg~ha^{-1}~y^{-1})*'\n'), seq(0,0.5,0.1), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
p3 <- bplot1(sampledata, 'slowTPloadkg', expression('Slow DRP Yield '~(kg~ha^{-1}~y^{-1})*'\n'), seq(0,0.5,0.1), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
p4 <- bplot1(sampledata, 'fastTNloadkg', expression('Fast NO3-N Yield '~(kg~ha^{-1}~y^{-1})*'\n'), seq(0,10,2), median_colour=NA, median_fill=fcol[3], median_alpha=0.2)
p5 <- bplot1(sampledata, 'medTNloadkg', expression('Medium NO3-N Yield '~(kg~ha^{-1}~y^{-1})*'\n'), seq(0,10,2), median_colour=NA, median_fill=mcol[3], median_alpha=0.2)
p6 <- bplot1(sampledata, 'slowTNloadkg', expression('Slow NO3-N Yield '~(kg~ha^{-1}~y^{-1})*'\n'), seq(0,10,2), median_colour=NA, median_fill=scol[3], median_alpha=0.2)
plotbox <- plot_grid(p1, p4, p2, p5, p3, p6, nrow=3, align="hv")
...
bplot2 <- function(sampledata, varname, ylabel, ybreaks, yref1, yref2){
  ...
  ggplot() +
    ...
    geom_path(aes(x=1:nruns, y=yref1), linetype=2, colour='black') +
    geom_path(aes(x=1:nruns, y=yref2), linetype=2, colour='black') +
    geom_vline(xintercept=xint, colour='grey') +
    # stat_boxplot(data=sampledata, mapping=aes_string(x='setseqf', y=varname), 
    #             geom="errorbar", width=0.35) +
...
refline1 <- rep(-0.25303,nruns)
refline2 <- rep(0.25303,nruns)
p1 <- bplot2(sampledata, 'auto1', 'R(1) DRP Calib\n', yscale, refline1, refline2)
# print(p1)
refline1 <- rep(-0.25303,nruns)
refline2 <- rep(0.25303,nruns)
p2 <- bplot2(sampledata, 'vauto1', 'R(1) DRP Valid\n', yscale, refline1, refline2)
p3 <- bplot2(sampledata, 'auto2', 'R(1) NO3-N Calib\n', yscale, refline1, refline2)
p4 <- bplot2(sampledata, 'vauto2', 'R(1) NO3-N Valid\n', yscale, refline1, refline2)
plotbox <- plot_grid(p1, p2, p3, p4, nrow=2, align="hv")
# print(plotbox)
...
p1 <- bplot(sampledata, 'grmse1', expression('GRMSE DRP Calib'~(mg~L^{-1})*'\n'), seq(0,0.07,0.01), 0.02)
p2 <- bplot(sampledata, 'vgrmse1', expression('GRMSE DRP Valid'~(mg~L^{-1})*'\n'), seq(0,0.07,0.01), 0.02)
p3 <- bplot(sampledata, 'grmse2', expression('GRMSE NO3-N Calib'~(mg~L^{-1})*'\n'), seq(0,0.7,0.1), 0.2)
p4 <- bplot(sampledata, 'vgrmse2', expression('GRMSE NO3-N Valid'~(mg~L^{-1})*'\n'), seq(0,0.7,0.1), 0.2)
# plotbox <- plot_grid(p1, p2, p3, p4, nrow=2, align="hv")
...
p1 <- barplot(runrecord, 'gelmanr', 'Gelman R\n', seq(0,max(runrecord$gelmanr)*1.1+0.5,0.5), 1.1, 1.0)
...
data <- vector("list", nrow(runlist))
i <- 1
for (i in seq_along(data)){
  temp <- runlist[i,] %>% 
    mutate(catch = catchfile)
  data[[i]] <- flowdata %>% 
    filter(catch == temp$catch) %>% 
    slice(temp$startcalib:temp$endcalib) %>% 
    mutate(
      setname = temp$setname,
      setseq = temp$setseq,
      shortname = temp$shortname,
      year = year(date),
      flow2 = flow/1000*3600*24/(temp$area*1000000)*1000 # l/s, km2, mm/d
    ) %>% 
    left_join(concdata %>% filter(catch == temp$catch) %>% mutate(TP=tp/1000,TN=tn/1000) %>% select(date, TP, TN), 
              by = "date")
}
data <- bind_rows(data) 
data2 <- data %>%
...
print(p1)
p2 <- bplot4(data2, 'meanTP', 'Annual Mean DRP', seq(0,0.3,0.1))
print(p2)
p3 <- bplot4(data2, 'meanTN', 'Annual Mean NO3-N', seq(0,3,1))
print(p3)
