...
parameters { // raw parameters to be adjusted
  ...
  real<lower=0.0, upper=1.0> chem1fastraw0;     
  ...
  real<lower=0.0, upper=1.0> chem1fastraw1;     
...
transformed parameters { // additional parameters to be used by the model
  ...
  for (n in (startrun+2):endvalid) {
    ...
    bach[n,6] = chem1fast0*(1-alpha)+chem1fast1*alpha;
    bach[n,7] = chem1med0 *(1-alpha)+chem1med1 *alpha;
    bach[n,8] = chem1slow0*(1-alpha)+chem1slow1*alpha;
    bach[n,9] = chem2fast0*(1-alpha)+chem2fast1*alpha;
    bach[n,10] = chem2med0 *(1-alpha)+chem2med1 *alpha;
    bach[n,11] = chem2slow0*(1-alpha)+chem2slow1*alpha;
...
generated quantities { // additional values of interest
  ...
  for (n in startcalib:endcalib) {
    ...
    beta = (n-366.0)/(5844.0-366.0);

    fastTPconc = chem1fast0*(1-beta)+chem1fast1*beta; // mg/L
    medTPconc = chem1med0*(1-beta)+chem1med1*beta;
    slowTPconc = chem1slow0*(1-beta)+chem1slow1*beta;

    fastTNconc = chem2fast0*(1-beta)+chem2fast1*beta;
    medTNconc = chem2med0*(1-beta)+chem2med1*beta;
    slowTNconc = chem2slow0*(1-beta)+chem2slow1*beta;

    fastTPload = fastTPload + (bach[n,1] - bach[n,2])*fastTPconc; // m3/s * mg/L
    medTPload = medTPload + (bach[n,2] - bach[n,3])*medTPconc;
    slowTPload = slowTPload + bach[n,3]*slowTPconc;

    fastTNload = fastTNload + (bach[n,1] - bach[n,2])*fastTNconc;
    medTNload = medTNload + (bach[n,2] - bach[n,3])*medTNconc;
    slowTNload = slowTNload + bach[n,3]*slowTNconc;
