...
transformed parameters { // additional parameters to be used by the model
  ...
  real slowrec;
  real ratio;
  real bach[ndate,17]; // need this here to calc auxiliaries later
  real alpha;
  ...
  // real<lower=-0.1*log(1-0.1), upper=-0.1*log(1-0.99)> medd1raw;       
  // real<lower=0.0, upper=1.0> slowb0raw;
  ...
  for (n in (startrun+2):endvalid) {
    ...
    // flow path concs
    alpha = (n-startcalib)*1.0/(endcalib-startcalib);
    ...
    bach[n,15] = -chem2fast0+chem2fast1+chem2fastb1*cos(pi()*alpha)*pi()+chem2fastb2*cos(2*pi()*alpha)*2*pi();
    bach[n,16] = -chem2med0 +chem2med1 +chem2medb1*cos(pi()*alpha)*pi() +chem2medb2*cos(2*pi()*alpha)*2*pi();
    bach[n,17] = -chem2slow0+chem2slow1+chem2slowb1*cos(pi()*alpha)*pi()+chem2slowb2*cos(2*pi()*alpha)*2*pi();
...
model { // compare model with data
  ...
  chem1fastraw0 ~ normal(0.1, priorwide); 
  chem1medraw0 ~ normal(0.1, priorwide); 
  chem1slowraw0 ~ normal(0.1, priorwide); 
  chem2fastraw0 ~ normal(0.1, priorwide);
  chem2medraw0 ~ normal(0.1, priorwide); 
  chem2slowraw0 ~ normal(0.1, priorwide);
  chem1fastraw1 ~ normal(0.1, priorwide); 
  chem1medraw1 ~ normal(0.1, priorwide); 
  chem1slowraw1 ~ normal(0.1, priorwide); 
  chem2fastraw1 ~ normal(0.1, priorwide);
  chem2medraw1 ~ normal(0.1, priorwide); 
  chem2slowraw1 ~ normal(0.1, priorwide);
